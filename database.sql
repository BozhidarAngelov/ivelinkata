...
-- 1. Create towns first (no dependencies)
...
-- 2. Create addresses (references towns)
...
-- 3. Create projects (referenced by employees_projects)
...
-- 4. Create employees (references addresses and departments, but departments not created yet)
-- We'll create employees without foreign keys first, then add them after departments
...
-- 5. Create departments (references employees)
...
-- 6. Create employees_projects (references employees and projects)
...
-- Now add the foreign key constraints that reference departments
...
ALTER TABLE `employees` ADD CONSTRAINT `fk_employees_addresses` FOREIGN KEY (`address_id`) REFERENCES `addresses` (`address_id`);
